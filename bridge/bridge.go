...
func (b *Bridge) addService(serviceId string, quiet bool) {
	...
	for key, port := range service.Endpoint.Ports {
		...
		ports[string(key)] = servicePortFromService(serviceId, service.Spec.Name, port.PublishedPort, port.TargetPort, port.Protocol, service.Spec.TaskTemplate.ContainerSpec.Env)
