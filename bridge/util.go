...
func serviceMetaDataSwarm(port ServicePortSwarm) (map[string]string, map[string]bool) {
	meta := port.Env
	log.Println("meta from Env:", meta)
	log.Println("Labels:", port.Labels)
...
func servicePortFromService(serviceId string, name string, publishedPort uint32, targetPort uint32, env []string, labels map[string]string) ServicePortSwarm {
	...
	return ServicePortSwarm{
		Port:        fmt.Sprint(publishedPort),
		PortType:    "tcp",
		ID:          serviceId,
		Name:        name,
